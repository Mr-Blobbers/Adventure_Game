...
void setup_class_defaults(ClassType c) {
    ...
    switch(c) { //A switch case to pick the desired character class and the stats of each class
    ...
    player.hp = player.max_hp; //Declaring the player max hp from the character class to player HP so it can be modified without changing the class itself 
}

void init_new_player() { //Initializing a new player
    printf("Enter your name: "); //Asking the player for their name
    read_line(player.name, sizeof(player.name)); //Reading the player input and saving it in player.name
    if(strlen(player.name)==0) strcpy(player.name, "Adventurer"); //If the name input was skipped the player will get the default name of "Adventurer"

    printf("Choose your class:\n"); //Asking the player to choose a class
    for(int i=0;i<CLASS_COUNT;++i) printf("[%d] %s\n", i+1, class_name(i)); //Output of a list with the class names and their corosponding number
    printf("> ");
    int pick = read_int(); //Reading the players input
    if(pick<1 || pick>CLASS_COUNT) pick=1; //Checking if the input was valid
    setup_class_defaults((ClassType)(pick-1)); 
    printf("Welcome, %s the %s! HP %d AC %d\n", player.name, class_name(player.cls), player.hp, player.ac); //Printing the player choices including Name, chosen Class, their HP and Armor class
}
